...
class Capture
{
	...
	int smooth_diameter = 7;	// changing to 0 causes lag (becuase this calcuates Sigma space)
...
public:
	...
	bool nextFrame()
	{
		...
		approxPoly_image = Mat::zeros(contours_image.size(), CV_8UC3);
		history_image = Mat::zeros(approxPoly_image.size(), CV_8UC3);
		//
			...
			if (contours_poly[i].size() == 4 && area > MIN_AREA_PIXELS && area < MAX_AREA_PIXELS) {		// if rectangle, judge angles ★ this must precede than angle judge for simple operation
				...
				if (isVertical){			
					...
					if (aspect_ratio < ERROR_RANGE_LENGTH_POSITIVE && aspect_ratio > ERROR_RANGE_LENGTH_NEGATIVE)
					{
						...
						// positive angle equals to 180 - abs(negative angle)  in rectangle
						if (angle_rotated < 0) {
							angle_rotated = 180 - abs(angle_rotated);
						}
						...
						ss1.append("(").append(to_string(frame_center.x)).append(", ").append(to_string(frame_center.y)).
							append("), Ar : ").append(format("%2.f", area / 1000)).
								append("k, DBC: ").append(format("%.2f", distance_between_centers));
						ss2.append(", An: ").append(format("%.2f", angle_rotated));
						...
						// putText Ar겹침 방지, 바뀌는 line 구분... ?..  ?.. zero로 초기화가 안되는 문제인가?.. 하나의 도형을 여러번 인식할 수도있는거같기도...?.. 나중에확인..
						if (count % 2 == 0) {
							frame_center.y += 40;
							...
							frame_center.y += 20;
							color = Scalar(50, 50, 150);
						}
						line(history_image, axis[0], axis[1], color);
						putText(history_image, ss2, Point(frame_center.x, frame_center.y), FONT_HERSHEY_PLAIN, 1, color); // 
						count++;
...
~.
	...
	?
		...
		? putText 겹침..... usleep 해도 똑같음...
			// for VNC viewer
			#include <unistd.h>
			#include <climits>
			//
			// sleep for VNC viewer frame (2 seconds)
			if (count > 60) {
				usleep(2000000);
			}
			count++;
